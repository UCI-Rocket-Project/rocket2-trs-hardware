...
TRS-GND runs **two radio instances** (TRS_RADIO × 2) for redundant uplink/downlink to TRS-ARM on the rocket.

---

## Secondary LoRa (SEEEDSTUDIOLORAERMINI)
...
Used for bridging or secondary telemetry path.
...
The Ethernet Xport converts UART data to TCP/IP, connecting TRS-GND into the test stand network. This allows GSE laptops and the GUI PC in the bunker to receive telemetry over the LAN.
